...
def load_top_one_probabilities(method):
    file_name = (
        f"results_data/{size}B-{method}-c{c}-m{m}-T{T}-"
        f"{prf}-{seed}-temp{temp}.pkl"
    )
    with open(file_name, "rb") as file:
        generation_data = pickle.load(file)

    try:
        top_probs = generation_data["watermark"]["top_probs"]
    except (KeyError, TypeError) as error:
        raise KeyError(
            f"{file_name} does not contain "
            "generation_data['watermark']['top_probs']."
        ) from error

    if hasattr(top_probs, "detach"):
        top_probs = top_probs.detach().cpu().numpy()

    top_probs = np.asarray(top_probs, dtype=float).reshape(-1)
    if top_probs.size == 0:
        raise ValueError(f"No top-one probabilities found in {file_name}.")
    if not np.all(np.isfinite(top_probs)):
        raise ValueError(f"Non-finite top-one probabilities found in {file_name}.")
    if np.any((top_probs < 0.0) | (top_probs > 1.0)):
        raise ValueError(
            f"Top-one probabilities outside [0, 1] found in {file_name}."
        )
...
top_one_methods = [
    ("gumbel", "Gumbel-max watermark", "tab:blue"),
    ("transform", "Inverse transform watermark", "tab:orange"),
]
top_one_bins = np.linspace(0.0, 1.0, 51)
top_one_axes = [ax[0][2], ax[1][2]]
